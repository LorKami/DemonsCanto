...
const FormsPages = () => {
	...
	const expresiones = {
		discord: /^.{3,32}#[0-9]{4}$$/, // Letras, numeros, guion y guion_bajo
		wallet: /^0x[a-fA-F0-9]{40}$/g, // Letras y espacios, pueden llevar acentos.
	...
	const onSubmit = async (e) => {
		...
		if (
			...
			fetch('https://demonsdb-1d911-default-rtdb.firebaseio.com/demons2.json').then(response => response.json()).then(data => {
				...
				console.log(found)
				if (found == "true") {
					alert("Error: This user has already been registered")
				} else {
					cambiarFormularioValido(true);
					cambiardiscord({ campo: '', valido: '' });
					cambiarwallet({ campo: '', valido: 'null' });

					const res = fetch("https://demonsdb-1d911-default-rtdb.firebaseio.com/demons2.json",
						{
							method: 'POST',
							headers: {
	...
	return (
		...
		<div className='FormSection'>
			<div className="FormTitle" data-aos="zoom-out" data-aos-delay="1000">
				<h2>DEMONS</h2>
			</div>
			<div className='FormInputs' data-aos="zoom-out" data-aos-delay="1500">
				<main>
			...
			<div className="LogoFooter" data-aos="fade-up-right" data-aos-delay="2000">
